      ...
      <form @submit.prevent="submit">
        ...
        <div class="mb-4">
          ...
          <div class="relative">
            ...
            />
            <ul v-if="pegawaiOptions.length > 0" class="absolute z-10 w-full mt-1 bg-white border rounded-lg shadow-lg max-h-60 overflow-auto">
              <li
          ...
          <div v-if="selectedPegawai" class="mt-2 p-3 bg-gray-50 rounded-lg">
            ...
            <div class="text-sm text-gray-600">{{ selectedPegawai.nip }} - {{ selectedPegawai.jabatan_nama }}</div>
          </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4 mb-4">
          <div>
            ...
            <input v-model.number="form.masa_kerja_tahun" type="number" min="0" max="32" class="w-full px-4 py-2 border rounded-lg" required />
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Masa Kerja (Bulan)</label>
            <input v-model.number="form.masa_kerja_bulan" type="number" min="0" max="11" class="w-full px-4 py-2 border rounded-lg" required />
          </div>
        </div>
        ...
        <div class="flex gap-4">
          ...
          <NuxtLink to="/kgb" class="px-6 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300">
            Batal
          </NuxtLink>
        </div>
      </form>
