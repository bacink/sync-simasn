...
export const kgbService = {
  async fetchKgbList(params?: { page?: number; status?: string; opd?: string; tahun?: string }): Promise<PaginatedResponse<KgbItem>> {
  ...
  async generateDraft(pegawaiId: number, masaKerjaTahun: number, masaKerjaBulan: number, jenisKgb: 'reguler' | 'penyesuaian'): Promise<ApiResponse<KgbItem & { calculation: KgbCalculation; snapshot: KgbSnapshot }>> {
    return api.post<ApiResponse<KgbItem & { calculation: KgbCalculation; snapshot: KgbSnapshot }>>('/kgb/generate', {
