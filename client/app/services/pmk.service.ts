...
export const pmkService = {
  async fetchPmkList(params?: {
    page?: number;
    status?: string;
    search?: string;
  }): Promise<PaginatedResponse<PmkItem>> {
  ...
  async uploadFile(
    file: File,
  ): Promise<ApiResponse<{ id: number; url: string; filename: string }>> {
    const formData = new FormData();
    formData.append("file", file);
    return api.postFormData<ApiResponse<{ id: number; url: string; filename: string }>>(
      "/upload",
      formData,
    );
  },
